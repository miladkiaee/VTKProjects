...
//  pointLocator->SetNumberOfPointsPerBucket(2);
  ...
  std::cout << "pointlocator" << std::endl;
  ...
  for (vtkIdType i = 0; i < order->GetNumberOfIds(); i++ ) {
    leftPoints2->InsertNextPoint(testPoints->GetPoint(order->GetId(i)));
  }

  leftPoints2->Print(std::cout);
  ...
  aCellArray->InsertNextCell(leftPoints2->GetNumberOfPoints());

  for ( vtkIdType i = 0; i < leftPoints2->GetNumberOfPoints(); i++ )
  {
    aCellArray->InsertCellPoint((order->GetId(i)));
  }
  aCellArray->Print(std::cout);

  for ( vtkIdType i = 0; i < leftPoints2->GetNumberOfPoints()-1; i++ )
  {
  ...
  vtkSmartPointer<vtkLine> lastline =
    vtkSmartPointer<vtkLine>::New();
  lastline->GetPointIds()->SetId ( 0, leftPoints2->GetNumberOfPoints()-1 );
  lastline->GetPointIds()->SetId ( 1, 0 );
  leftLines->InsertNextCell ( lastline );

  leftLines->Print(std::cout);
